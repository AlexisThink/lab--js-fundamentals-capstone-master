...
function pluck(object, palabra){
    ...
    if (parametro === "name"){
        for(var i = 0; i < newArray.length; i++){
            ...
            finalList.push(newArray[i].name);   
        }
    } else if (parametro === "age"){
        for (var j = 0; j < newArray.length; j++){
            finalList.push(newArray[j].age);
        }
    } else if(parametro === "hairStyle"){
        for(var h = 0; h < newArray.length; h++){
            finalList.push(newArray[h].hairStyle);
        }
    }
    return finalList;
}
